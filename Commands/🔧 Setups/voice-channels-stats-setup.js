...
module.exports = {
    ...
    .addSubcommand(command => command.setName('total-set').setDescription('Sets your total members voice channel.').addChannelOption(option => option.setName('voice-channel').setDescription('Specified voice channel wll be your total members voice channel.').setRequired(true).addChannelTypes(ChannelType.GuildVoice))  .addStringOption(option => option.setName("channel-name").setDescription("Name of the channel").setRequired(true)))
    .addSubcommand(command => command.setName('total-remove').setDescription('Removes your total members VC.'))
    .addSubcommand(command => command.setName('bot-set').setDescription('Sets your total bots voice channel.').addChannelOption(option => option.setName('voice-channel').setDescription('Specified voice channel wll be your total bots voice channel.').setRequired(true).addChannelTypes(ChannelType.GuildVoice)).addStringOption(option => option.setName("channel-name").setDescription("Name of the channel").setRequired(true)))
        ...
        switch (sub) {
            ...
            if (!voicedata) {
                ...
                await voiceschema.create({
                    ...
                    ChannelName: interaction.options.getString("channel-name")
                })
                const TotalChannelName1 = interaction.options.getString("channel-name")

                voicetotalchannel.setName(`${TotalChannelName1} ${interaction.guild.memberCount}`).catch(err);
            ...
            if (!botdata) {
                ...
                const BotChannelName1 = interaction.options.getString("channel-name")

                botguildchannel.setName(`${BotChannelName1} ${botcount}`).catch(err);
