...
module.exports = {
    ...
    async execute(bot, interaction) {
        ...
        TicketSchema.findOne({GuildID: guild.id, ChannelID: channel.id}, async (err, data) => {
            if (err) throw err;
            if (!data) return interaction.reply({embeds: [embed.setColor('Red').setDescription('Veuillez réessayer plus tard.')], ephemeral: true}).catch(error => {return});
            ...
            interaction.channel.send({embeds: [embed.setColor('Green').setDescription('<@' + interaction.values[0] + '>' + ' ' + 'a deja été ajouté au ticket.')]}).catch(error => {return});
            data.save();
            }else {
            data.MembersID.remove(interaction.values[0]);
            channel.permissionOverwrites.delete(interaction.values[0]).catch(error => {return});
            interaction.channel.send({embeds: [embed.setColor('Green').setDescription('<@' + interaction.values[0] + '>' + ' ' + 'a été enlevé du ticket.')]}).catch(error => {return});
            data.save();
            }
    })
    }
